...
当前是 **BUILD 222 网页内测版**，请先用测试资料体验，功能与使用条件见[更新记录](CHANGELOG.md#使用说明)。云平台、域名、模型和语音可能收费；网页与后台在你所在网络中的可达性需要分别确认。
...
系统伙伴入口：手机助手 → 系统伙伴。可从五个模板创建或自定义，使用已选 API 模型连续对话，保存和修订共同记忆，并绑定准备好的平行世界故事。切换人格会保留旧对话；任务判定、道具执行与 MCP 尚未接入。
